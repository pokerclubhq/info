...
- **Script** — An EZScript expression that calculates a player's points. The built-in formula curves the award logarithmically by finishing place, and splits a tie across the places it spans the way payouts do. The simplest alternative, `totalPlayerCount - playerId.toPlayer().place + 1`, just pays last place 1 point and each place above it one more.
...
The same formula is what [Club stats](/info/club-stats/) sums across a season, and what the [Points widget](/info/widgets/#points) puts on the timer screen when a tournament ends. Set it once and all three agree.
