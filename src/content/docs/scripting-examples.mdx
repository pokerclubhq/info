...
For a fixed scale instead of a curve, list the awards and look them up by place:
...
Two players tied in 4th now score 63 each, (65 + 60) / 2 rounded, instead of 65 apiece. Every formula above has the same blind spot, the built-in curve included, so fold `placeCount` in wherever a tie should be shared rather than doubled.
