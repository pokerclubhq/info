...
This is the built-in **near end of level** alert. Without the `previous` clause it would fire sixty times.
...
:::caution
A condition that mentions `previous` can't be judged from the editor's result line. In the editor `previous` is the *same* snapshot as the present, so `secondsLeftInLevel > previous.secondsLeftInLevel` reads false and `secondsLeftInLevel == previous.secondsLeftInLevel` reads true — neither tells you what will happen with the timer running.

The **Preview** button doesn't help either: it plays the alert's actions outright, without checking the condition. Run the timer to test a trigger.
:::
