...
:::note
Alerts keep firing for as long as the tournament is open, on any of its screens. Settings, Blinds, Players, Tables, Payouts, Log, Stats, and Alerts all sit on top of the timer, so moving to one of them silences nothing. Only leaving the tournament stops them.

They do stay quiet while the timer is paused, and they're skipped when the app wakes from being backgrounded, so a backlog of stale announcements never plays at once.
:::
...
:::caution
There's no earlier moment in the editor, so `changed(...)` and `becameTrue(...)` answer as though the change they describe had just happened, and the result line adds *(assuming it just changed)* to say so. The rest of the condition is still judged on where the tournament actually is, which is the part worth checking: `becameTrue(secondsLeftInLevel <= 60) && currentLevel.isBlindLevel` reads false with five minutes left on a blind level, and true with thirty seconds left.

Two things it still can't judge. A comparison written with `was(...)` directly, since there's no older value to give it — `currentPlayerCount < was(currentPlayerCount)` reads false whatever you do. And anything using the older `previous` variable, for the same reason.

**Preview** doesn't test the condition either: it plays the alert's actions outright. Run the timer to see a trigger fire.
:::
...
:::note
Conditions written before these existed used a `previous` variable — `levelIndex > previous.levelIndex` for what `changed(levelIndex)` says now. It still works and doesn't need changing. The newer functions are worth reaching for in anything new, since they name what you're asking about rather than routing it through a second name for the whole tournament.
:::
...
<Image src={alerts00001} alt="The Choose voice sheet, with voices grouped under English - United States, English - Australia and English - United Kingdom headings. Samantha and Daniel come before the Google voices, which carry a cloud icon." width={320} />
...
Two things worth knowing about it. It applies only to the device you set it on, which is how you keep a phone that scanned a [QR code](/info/qr-sharing/) from announcing along with the main timer. And it resets to unmuted each time the app starts, so a device you want kept quiet needs muting again after a restart.
