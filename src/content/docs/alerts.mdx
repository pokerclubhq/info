...
:::caution
The condition is checked **every second** while the timer is running. Every time the condition evaluates to true, the alert triggers. This means a simple condition like `currentLevel.bigBlind >= 100` would fire repeatedly — once per second — for the entire duration that the big blind is 100 or more. To avoid this, use the `previous` variable to detect the exact moment a value changes.
:::
...
- **Text** — The announcement to speak. Supports variable interpolation with double curly braces, for example: `The big blind is now {{ currentLevel.bigBlind }}` or `{{ currentPlayerCount }} players remain`
