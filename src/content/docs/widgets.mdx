---
title: Widgets
description: The Poker Club HQ widget gallery — the fourteen built-in timer blocks, why a widget looks empty, and how to save, rename, and reuse your own.
---
...
Fourteen widgets ship with the app. Most of them hide themselves until the tournament actually has something to show, so it's safe to add them at any point — a Payouts widget added before anyone has bought in simply stays invisible until money comes in.
...
| **Leaderboard** | The season standings of the tournament's folder: the top ten players by [Club stats](/info/club-stats/) points, with their totals | The folder has a completed tournament |
...
| **Color up!** | Which chip denominations to take off the table, drawn as the chips themselves | It's a color-up level (a break, by default) and the blinds have outgrown a chip since the last one |
...
The **Leaderboard** widget puts the [Club stats](/info/club-stats/) leaderboard on the timer: the players of the tournament's folder (subfolders included), most season points first, in two aligned columns. It lists the top ten and marks a longer field with a trailing "…".

Only completed tournaments count, as on the Club stats screen. During a game it shows the standings coming into tonight, and when the game ends, tonight's results join them. A tournament that isn't in a folder has no season, so the widget stays hidden.

It isn't part of the default layout, so add it from the gallery wherever you want it. Any text you write can read the same numbers from `clubStats`. See [Tournament data](/info/scripting-data/#club-stats).
...
As the blinds climb, the smallest chips stop being needed. Nobody makes change for a 25 when the small blind is 500. The **Color up!** widget watches for that and names the denominations that became retirable since the *previous* color-up point: a compact orange banner reading **Color up!** followed by the chips themselves, drawn as your actual chip images.

Where those points fall is up to you, under **Color up chips** in the tournament's [settings](/info/setup/). By default they're your breaks, which is where chips usually come off the table. Set it to **At any level** and every level is one, so the banner names a chip at the level it actually retires instead of waiting for the next break. A tournament with no breaks at all works that way already, since there's no break to wait for.
...
| **Points** | `isPostgame` |
| **Leaderboard** | `clubStats.players.isNotEmpty` |
| **Payouts** | `totalCollected > 0` |
| **High hand** | `hasHighHand` |
| **Chip leaders** | `chipLeaders.isNotEmpty && !isPostgame` |
| **Timer** | `!isPostgame` |
| **Color up!** | `autoColorUp && isColorUpLevel && colorUpChips.isNotEmpty` |
...
| **Leaderboard** | `leaderboard title`, `leaderboard names`, `leaderboard totals` | `clubStats.players.isNotEmpty` |
...
| **Color up!** | `color up title`, `color ups` | `autoColorUp && isColorUpLevel && colorUpChips.isNotEmpty` |
