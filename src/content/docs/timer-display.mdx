...
The **+** buttons in edit mode open a gallery of ready-made content widgets you can drop into the layout — prebuilt cells that show a particular piece of tournament information, so you don't have to assemble them from scratch.

### Chip leader

The **Chip leader** widget shows who currently has the most chips, drawn from the chip counts players report themselves. When players connect to a shared tournament and report their current chips (see [Player profile updates](/info/player-updates/)), the app tracks each active player's latest count and picks the leader; the widget shows their name and stack, and hides itself when no chip counts have been reported yet.

For custom layouts and [alerts](/info/alerts/), the same information is available as the `chipLeaderPlayer`, `chipLeaderChips`, `chipLeaderDt`, and `hasChipLeader` script variables.
